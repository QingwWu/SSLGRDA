...
def ranks(y_true, y_pred, pos_label=1.0):
    ...
    """
    posnub = np.where(y_true.flatten()==0)
    allranks=[]
    for y_ in  range(y_true.shape[0]):
      y_prob_neg = y_pred[posnub] 
      if y_true[y_] != 0:
          y_prob_neg = np.hstack((y_pred[y_],y_prob_neg)).flatten()
          p_sort = np.argsort(-y_prob_neg)   
          rank = np.where(p_sort == 0)[0][0]
          allranks.append(1+rank)
    return np.array(allranks)
